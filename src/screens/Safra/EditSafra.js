...
export default function EditSafra({ route, navigation }) {
    ...
    const key = descriptionSafra


    //consts for BD connection

    async function newResponseUPDATE(descriptionSafra, dateStartSafra, dateEndSafra, observationSafra, descriptionCulture, dateStartCulture, observationCulture) {
        await firebase.database().ref(`users/${uid}/safra/${key}`).update({
            descriptionSafra: descriptionSafra,
            dateStartSafra: dateStartSafra,
            dateEndSafra: dateEndSafra,
            observationSafra: observationSafra,
    ...
    const createUpdateSucessAlert = () =>
        Alert.alert(
            "Parabéns!",
            "Sua safra foi alterada com sucesso.",
            [
                { text: "Ok", onPress: () => navigation.navigate('Safra') }
            ]
        )

    async function newResponseDELETE() {
        await firebase.database().ref(`users/${uid}/safra/${descriptionSafra}`).remove()
            .then((value) => {
    ...
    const createDeleteSucessAlert = () =>
        Alert.alert(
            "Parabéns!",
            "Sua safra foi excluida com sucesso.",
            [
                { text: "Ok", onPress: () => navigation.navigate('Safra') }
            ]
        )
    ...
    const createDeleteAlert = () =>
        Alert.alert(
            "Atenção!",
            "Deseja mesmo excluir essa safra?",
            [
    ...
    function pushData(descriptionSafra) {
        navigation.navigate('EditCoordinates', { descriptionSafra, uid })
    }
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                <View style={styles.appBar}>
                    <Appbar.Header style={styles.appBarStyle}>
                        ...
                        <Appbar.Content title="Editar informações da safra" color='#ffff' />
                        <Appbar.Action color='#fff' icon="delete" onPress={() => createDeleteAlert()} />
                    </Appbar.Header>
                </View>

                <Formik
                    initialValues={{ descriptionSafra: descriptionSafra, dateStartSafra: dateStartSafra, dateEndSafra: dateEndSafra, observationSafra: observationSafra, descriptionCulture: descriptionCulture, dateStartCulture: dateStartCulture, observationCulture: observationCulture }}
